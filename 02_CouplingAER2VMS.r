...
##!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!##
 ...
 setwd(file.path("C:","Users","fbas","Documents", "FishSpatOverlayTool"))
...
# READ SOME VMS DATA
 ...
 load(file=file.path(RinputPath, "VMS_DATA", "prodT53all_agg_FB.Rdata"))  # SeaWise
 head(prodT53all_agg)
 ...
 library(stringr)
 temp <- as.data.frame(str_split_fixed(prodT53all_agg$MetierL6,"_",3))  
 prodT53all_agg$gear       <- temp[,1]
 prodT53all_agg$assemblage <- temp[,2]
 prodT53all_agg$meshsize   <- temp[,3]
 ...
 prodT53all_agg <- left_join(prodT53all_agg, dcf_gears, by="gear")


 # Retrieve the sub_reg? 
  data(ICESareas) # in vmstools
  sq         <- cbind.data.frame(unique(prodT53all_agg$Csquare), vmstools::CSquare2LonLat(unique(prodT53all_agg$Csquare), 0.05))
 ...
 prodT53all_agg <- left_join(prodT53all_agg, sq, by=c("Csquare"))

 colnames(prodT53all_agg)[colnames(prodT53all_agg)=="VesselLengthRange"] <- "vessel_length" 
 ...
 prodT53all_agg$fs_name     <- paste(prodT53all_agg$fishing_tech, prodT53all_agg$vessel_length, prodT53all_agg$sub_reg, sep="_") # vms

 vms <- prodT53all_agg[, .(FishingHour = sum(FishingHour, na.rm=T),
                                                 kWFishingHour = sum(kWFishingHour, na.rm=T)),
                                             by=c("Year", "Csquare", "icesname", "SI_LATI", "SI_LONG",  "MetierL6", "vessel_length", "fishing_tech", "sub_reg")]  # here the month dim is lost
 vms$year <- vms$Year
 ...
 # add primary keys before merging - adapt the key on the fly to avoid loosing effort or landings....
   ...
   for(y in years)
   {
     # Add a default key (i.e. year-quarter-Level5-icesrect)
      ...
      vms_y$key               <- paste(vms_y$year, vms_y$vessel_length, vms_y$fishing_tech, vms_y$sub_reg, sep="_")  # full key on vms
      aer_y                   <- stecf_fleetdata_with_kiloandeffort_andcostratios_nao[year==y,]
      aer_y$key               <- paste(aer_y$year,  aer_y$vessel_length, aer_y$fishing_tech, aer_y$sub_reg, sep="_") # full key on aer
      aer_y$chunk <- 0
      vms_y$chunk <- 0
      m1                      <- unique(vms_y$key)
      m2                      <- unique(aer_y$key)
     ...
     {
      aer_y_leftover$key       <- paste(aer_y_leftover$year, aer_y_leftover$vessel_length, aer_y_leftover$fishing_tech,  sep="_") # aer
      vms_y_leftover$key       <- paste(vms_y_leftover$year, vms_y_leftover$vessel_length, vms_y_leftover$fishing_tech, sep="_")  # vms
      ...
      vms_y_leftover      <- vms_y_leftover[,lapply(.SD, sum, na.rm=TRUE), 
                                   ...
                                   by=c("Csquare", "icesname", "SI_LATI", "SI_LONG", "MetierL6", "vessel_length", "fishing_tech", "year", "key")]    
   ...
   "year, vessel_length, fishing_tech, sub_reg"=d(vms_y_main[  ,.(vmstotfishhours =sum(FishingHour )),]),        #  year, vessel_length, fishing_tech, sub_reg  # full key 
   "year, vessel_length, fishing_tech"=d(vms_y_leftover[  ,.(vmstotfishhours =sum(FishingHour )),]),    #  year, vessel_length, fishing_tech
   ...
   "year, vessel_length, fishing_tech, sub_reg"=d(vms_y_main[  ,.(vmstotKwfishhours=sum(an(kWFishingHour))),]),        #  year, vessel_length, fishing_tech, sub_reg  # full key  
   "year, vessel_length, fishing_tech"=d(vms_y_leftover[  ,.(vmstotKwfishhours=sum(an(kWFishingHour))),]),    #  year, vessel_length, fishing_tech 
...
aer_all_y[, .(weight = sum(an(weight), na.rm=T),
 ...
 # caution with Inf on aerECOkwFishingdays_perregion....REDO AFTER SCALING DOWN LARGE NUMBERS IF IT IS THE CASE.
 aer_all_y[country_code=="FRA" & vessel_length=="VL1218" & year=="2020",]
 ...
 for (y in as.character(years))
 {
  ...
  if(FALSE){
    ...
    temp <- as.data.frame(stringr::str_split_fixed(as.character(unlist(aer_y  [,"key"])),"_",4))  
    aer_y[, "vessel_length"] <- temp[,2]
    aer_y[, "fishing_tech"] <- temp[,3]
   ...
   a_dispatcher_key                                       <-  merged$country_share_effort * merged$share_effort_level6 * merged$share_effort_level6_csquare    # because VMS are metier-based data on Csquare  
   ...
   # check
   library(stringr)
   temp <- as.data.frame(stringr::str_split_fixed(as.character(unlist(merged[,"key"])),"_",4))  
   dd <- merged[merged$country_code=="BEL" & temp[,3]=="TBB" & temp[,2]=="VL2440" & merged$y=="2019",]                              
   ...
   temp <- as.data.frame(stringr::str_split_fixed(as.character(unlist(aer_y[,"key"])),"_",4))  
   dd <- aer_y[aer_y$country_code=="BEL" & temp[,3]=="TBB" & temp[,2]=="VL2440" & aer_y$y=="2019",]
  ...
  if(FALSE){
    nonvariablevars <- AER_nonvariablevars[AER_nonvariablevars$supra_reg=="NAO",]
    temp <- as.data.frame(stringr::str_split_fixed(as.character(unlist(merged  [,"key"])),"_",4))  
    merged[, "vessel_length"] <- temp[,2]
    merged[, "fishing_tech"] <- temp[,3]
    merged[, "sub_reg"] <- temp[,4]
    ...
    ddd$GVAish <- (an(ddd$landings_aer_in_ctry_level6_csquare) *  # landing kg  * price
                      (an(ddd$value_aer_in_ctry_level6_csquare)/an(ddd$landings_aer_in_ctry_level6_csquare))) - an(ddd$varcosts)  # landing value minus var costs  (-ish because here we lack oth income and unpaid labour...)
 ...
 # check 
 dispatched_aer_ally[country_code=="FRA"  & year=="2020",]
 ...
 library(readr)
 dd <- knitr::kable(as.data.frame(dd), format = "html")
 readr::write_file(dd, file.path(ROutputPathToDatasets, "an_overall_check1.html")) 
 ...
 save(dispatched_aer_ally, file=file.path(ROutputPathToDatasets, "dispatched_aer_vars_on_vms_effort_csquares_ally.RData")) 
...
library(data.table)
 ...
 load(file=file.path(file.path(ROutputPathToDatasets, "dispatched_aer_vars_on_vms_effort_csquares_ally.RData")))  # get "dispatched_aer_ally"
 ...
 distr         <- dplyr::left_join(dispatched_aer_ally, sq, by="Csquare")
  ...
  ## !!!export!!! ##
 #save(distr, file=file.path(ROutputPathToDatasets, "distr_AER2022_on_VMS.RData"))
...
## aggregate per Csquare, year and key-country
 ...
 # check 
 ss <- as.character(unlist(unique(distr_allsp[country_code=="FRA"  & year=="2018", "key"])))
 ss[grep("27.3.a", ss)]
 ...
 load(file=file.path(filename))  #  get AERothvars
  ...
  library(stringr)
  temp <- as.data.frame(stringr::str_split_fixed(as.character(unlist(distr_allsp[,"key"])),"_",4))  
  distr_allsp[,"fs"] <- paste0(unlist(c(distr_allsp[, "country_code"])), "_", temp[,3], "_", temp[,2])
...
#=> to be computed on the final merged dataset, then to be recomputed after applying displacement scenarios (e.g. based on LPUEs-costs...) changing the income from landings and the costs
 ...
 distr_allsp$GrossProfit <- distr_allsp$GVA - an(distr_allsp$personnelcosts_in_ctry_level6_csquare)
  ...
  ## !!!export!!! ##
 save(distr_allsp, file=file.path(ROutputPathToDatasets, "distr_allsp_from_AER_coupled_to_VMS.RData"))
 ...
 ## !!!export!!! ##
 save(agg_eco_fs, file=file.path(ROutputPathToDatasets, "agg_vms_aer_eco_fs.RData"))
...
##!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!#
 ...
 #----------------------
 # load
 load(file=file.path(ROutputPathToDatasets, "distr_allsp_from_AER_coupled_to_VMS.RData"))  # get distr_allsp i.e. all species aggregated (if AER)
 library(data.table)
...
if(FALSE){ 
 ...
 a_var <- distr_allsp[, .(a_var_to_plot=sum(lpue_csquare_vms_kgperfhour, na.rm=TRUE)), by=c("Csquare", "SI_LATI", "SI_LONG")]
 a_var <- a_var[ SI_LATI>50 &  SI_LATI<62 &  SI_LONG>-2  &  SI_LONG<10 , ] # in North Sea only
...
my_raster_export <- function(master_dt, slave_dt, effort_only=FALSE){     
  ...
  #distr_with_grid[key=="2018_VL1218_DTS_27.8.a", ]
  ...
  if("fs" %in% colnames(distr_with_grid)) {
     #nby_per_fs <- distr_with_grid[,.(nby=length(unique(year))),by=fs]
     #distr_with_grid <- merge(distr_with_grid, nby_per_fs, by="fs")
     nby_per_grID <- distr_with_grid[,.(nby=length(unique(year))),by=grID]
     distr_with_grid <- merge(distr_with_grid, nby_per_grID, by="grID")
     }  else{
     distr_with_grid$nby <- length(unique(master_dt$year))
     }
...
## CALLS------------------------------
  ...
  a_width <- 3000 ;  a_height <- 4000
   tiff(filename=file.path(getwd(), "OUTCOME_FISHERIES_DISTR_VMS", "all_metiers", paste0("ICES_VMS_log_of_FishingHour_mapped_for_2018_2021.tif")), width = a_width, height = a_height,
 ...
 # or plot VMS per DCF metier, all years
  a_df                 <- as.data.frame(vms_effort_nao)
  for (fs in unlist(unique(vms_effort_nao[,"MetierL6"])))
  {
   cat(paste0(fs, "\n"))
    
     vms_effort_nao_this_fs        <- data.table(a_df[a_df$MetierL6 %in% fs,])
     fs <- gsub(">=", "o", fs)
     fs <- gsub(">", "o", fs)               
     fs <- gsub("<=", "u", fs)
     fs <- gsub("<", "u", fs)
     filepath             <- file.path(getwd(), "OUTCOME_FISHERIES_DISTR_VMS", fs, "2018_2021")
    
 
     dir.create(filepath, recursive=TRUE)
     my_raster_export(master_dt=vms_effort_nao, slave_dt=vms_effort_nao_this_fs)
     # check:
 ...
 # VMS-AER coupled, all metiers, all years
  ...
  for (fs in unlist(unique(distr_allsp[,"fs"])))
  {
   cat(paste0(fs, "\n"))
     ...
     seawise <- TRUE
     if(seawise){
     #TODO: A FIX TO AVOID MISPLACED EFFORT DUE TO BAD MERGING WHEN MISSING AREA INFO IN INPUT: REDISPATCH ALL VARS FROM NON-AREA INFORMED DATA TOWARDS WELL-INFORMED AREAS  
     # UNFORTUNATELY, MISSING COUNTRY IN THE VMS INPUT DOES NOT HELP BEING ACCURATE IN FS SPECIFIC EFFORT ALLOCATION.....
     allkeys <- unique(distr_this_fs$key) 
     temp <- as.data.frame(str_split_fixed(allkeys,"_",4))  
     allkeys_to_remove <-  allkeys[which(temp[,4]=="")] # the ones without the area info....(otherwise likely artificial allocation of effort where they were none, as the result of the robust merging)
     if(!all(allkeys %in% allkeys_to_remove) && length(allkeys_to_remove)!=0){
        some_cols_to_sum <- c("FishingHour", "daysatsea_aer_in_ctry_level6_csquare", "landings_aer_in_ctry_level6_csquare", "value_aer_in_ctry_level6_csquare",
             "varcosts_in_ctry_level6_csquare", "oth_non_var_costs_in_csquare",
          "other_income_in_csquare" ,"unpaid_labour_in_csquare", 
          "personnelcosts_in_ctry_level6_csquare",
           "KwFishingdays_aer_in_ctry_level6_csquare")   
        a_sum <- distr_this_fs[key %in% allkeys_to_remove[1:(length(allkeys_to_remove)-1)],
                           lapply(.SD, sum, na.rm=TRUE),
             .SDcols=some_cols_to_sum,]
        distr_this_fs <- as.data.frame(distr_this_fs)    
        distr_this_fs <- distr_this_fs[!distr_this_fs$key %in% allkeys_to_remove[1:(length(allkeys_to_remove)-1)],] # keep at least last year key given issue identified with e.g. "FRA_DTS_VL1218" 
        xx <- (as.matrix(a_sum)/ nrow(as.matrix(distr_this_fs[, some_cols_to_sum])) ) 
        distr_this_fs[, some_cols_to_sum] <-  sweep(as.matrix(distr_this_fs[, some_cols_to_sum]), 2,  xx, FUN=function(x,y) x+y)  # redispatch evenly from the removed keys to the remaining keys
        distr_this_fs <- data.table(distr_this_fs)
        } # end allkeystoremove are not allkeys
     } # end seawise
     
     #distr_this_fs_this_y <- distr_this_fs[distr_this_fs$year=="2019",]
     filepath             <- file.path(getwd(), "OUTCOME_FISHERIES_DISTR_VMS_AER", fs, "2018_2021")
